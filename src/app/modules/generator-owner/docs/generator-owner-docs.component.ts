...
import { AfterViewInit, Component, DestroyRef, ElementRef, OnDestroy, OnInit, NgZone, ViewChild, ViewEncapsulation, inject, ChangeDetectionStrategy, ChangeDetectorRef } from '@angular/core';
...
import { DocsLanguage, DocsText, DocsTopic, GENERATOR_OWNER_DOCS } from './generator-owner-docs.data';
...
type GoDocsMouseEvent = MouseEvent & {
    __goDocsLinkHandled?: boolean;
};

@Component({
    selector: 'app-generator-owner-docs',
    standalone: true,
    imports: [CommonModule, FormsModule, Button, DrawerModule, IconField, InputIcon, InputText, Select, Tag, Tooltip],
    templateUrl: './generator-owner-docs.component.html',
    styleUrl: './generator-owner-docs.component.scss',
    encapsulation: ViewEncapsulation.None,
    // Do not use OnPush if you have many reactive updates, but it's fine.
    changeDetection: ChangeDetectionStrategy.OnPush
})
export class GeneratorOwnerDocsComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    readonly docs = GENERATOR_OWNER_DOCS;
    ...
    ngAfterViewInit(): void {
        ...
        // Attach direct click handlers to anchors (bypass event propagation issues)
        this.attachDirectAnchorHandlers();

        // Observe DOM changes to re-attach handlers for new anchors
    ...
    get pageTitle(): string {
        return this.isArabic ? 'دليل مالك المولّد' : 'Generator Owner Docs';
    }
    ...
    get articleHtml(): SafeHtml {
        // Return cached version to avoid infinite re-renders
    ...
    selectDoc(doc: DocsTopic, options: SelectDocOptions = {}): void {
        ...
        requestAnimationFrame(() => {
            requestAnimationFrame(() => {
                ...
                // Re-attach direct handlers after DOM update
                this.attachDirectAnchorHandlers();
            });
        });
    }
    ...
    openInApp(doc: DocsTopic | null = this.selectedDoc): void {
        ...
        this.router.navigate(['/app', 'generator-owner', doc.appRoute]);
    }
    ...
    onArticleClick(event: MouseEvent): void {
        // Not needed because we use direct handlers, but keep for safety
        this.handleArticleAnchorClick(event as GoDocsMouseEvent);
    }

    // ------------------------------------------------------------
    // Direct anchor handler (bypasses event propagation issues)
    // ------------------------------------------------------------
    private attachDirectAnchorHandlers(): void {
        ...
        anchors.forEach((anchor) => {
            if ((anchor as any).__docsHandlerAttached) return;
            (anchor as any).__docsHandlerAttached = true;
    ...
    private handleArticleAnchorClick(event: GoDocsMouseEvent): void {
        if (event.__goDocsLinkHandled) return;
        event.__goDocsLinkHandled = true;
        ...
        if (!targetDoc) return; // not a doc link
    ...
    // ------------------------------------------------------------
    // Tooltip methods (unchanged, but ensure they don't cause re-renders)
    // ------------------------------------------------------------
    ...
    private getBestAnchorRect(anchor: HTMLAnchorElement, event?: PointerEvent): DOMRect {
        ...
        const containingRect = rects.find((r) => pointX >= r.left && pointX <= r.right && pointY >= r.top && pointY <= r.bottom);
    ...
    private searchMatches(doc: DocsTopic, search: string): boolean {
        ...
        const haystack = [doc.title.en, doc.title.ar, doc.group.en, doc.group.ar, ...doc.keywords, ...doc.sections.flatMap((s) => [s.title.en, s.title.ar]), contentText].join(' ').toLowerCase();
