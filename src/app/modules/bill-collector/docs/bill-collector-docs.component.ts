...
import { BILL_COLLECTOR_DOCS, DocsLanguage, DocsText, DocsTopic } from './bill-collector-docs.data';
...
type BcDocsMouseEvent = MouseEvent & {
    __bcDocsLinkHandled?: boolean;
};

@Component({
    selector: 'app-bill-collector-docs',
    standalone: true,
    imports: [CommonModule, FormsModule, Button, DrawerModule, IconField, InputIcon, InputText, Select, Tag, Tooltip],
    templateUrl: './bill-collector-docs.component.html',
    styleUrl: './bill-collector-docs.component.scss',
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush
})
export class BillCollectorDocsComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    readonly docs = BILL_COLLECTOR_DOCS;
    ...
    get pageTitle(): string {
        return this.isArabic ? 'دليل محصل الفواتير' : 'Bill Collector Docs';
    }
    ...
    openInApp(doc: DocsTopic | null = this.selectedDoc): void {
        ...
        this.router.navigate(['/app', 'bill-collector', ...routeSegments]);
    }
    ...
    onArticleClick(event: MouseEvent): void {
        this.handleArticleAnchorClick(event as BcDocsMouseEvent);
    }
    ...
    private handleArticleAnchorClick(event: BcDocsMouseEvent): void {
        if (event.__bcDocsLinkHandled) return;
        event.__bcDocsLinkHandled = true;
